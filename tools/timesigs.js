//===================================================//
//Time Signature Changes for MuseLab
//forked from jeetee/MuseScore_Timesig_Enforcer
//v 1.0
//changelog:
//===================================================//

function handleTimeSigChanges(m, endTick) {
	if (m.firstSegment.tick < endTick) {
		if (m.timesigActual.ticks < m.timesigNominal.ticks) {
			...
			if (curScore.selection.select(insertBeforeElement)) {
				...
				console.log("Failed to select element to insert before");
			}
			//"reload" the score
			curScore.endCmd();
		...
		if (m.timesigActual.ticks > m.timesigNominal.ticks) {
			...
			if (curScore.selection.select(chordRestToClip)) {
				...
				console.log("failed to select element to clip");
			}
